<template>
  <div v-if="$page.props.auth.user">
    ...
    <div class="mb-4">
      <inertia-link class="flex items-center group py-3" :href="route('rents')">
        <icon name="book" class="w-4 h-4 mr-2" :class="isUrl('rents') ? 'fill-white' : 'fill-yellow-400 group-hover:fill-white'" />
        <div :class="isUrl('rents') ? 'text-white' : 'text-yellow-300 group-hover:text-white'">Prosedur Peminjaman</div>
      </inertia-link> 
    </div>
    <!-- <div class="mb-4">
      <inertia-link class="flex items-center group py-3" :href="route('docs')">
